...
| M5 Avvio di Android | **in progress** (our AOSP 15 image, ADR 0022; in Chrome, ADR 0028: home screen drawn 44.5 min after a cold start with 2 GiB, adb from the page, a dropped APK installed, opened and reacting to a click; long tests `VETRO_ANDROID=1`, not in CI yet) | home in Chrome, adb install di un APK |
| M6 Snapshot e installazione | **in progress** (full machine save/restore, ADR 0015; persistent disks and cached snapshots in the browser, ADR 0017; Android: chunked snapshots, second start in Chrome ready in 4.8–5.0 s, ADR 0028; exit command not in CI yet) | home < 15 s da snapshot, APK trascinato |
