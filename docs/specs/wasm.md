...
- 6 (M6): overlay copy-on-write persistente dei dischi (`vetro_overlay_*`,
  ADR 0016); `vetro_snapshot_restore` segna gli overlay per il confronto
  completo.
...
## Persistenza in JavaScript (`web/node/persist.mjs`, M6, ADR 0016)
...
- Persistenza nel Worker (ADR 0016): per ogni disco scrivibile un overlay
